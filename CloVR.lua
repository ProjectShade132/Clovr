...
yes. this is a very watered down ver of wyvern reanimate
...
local FlingOptions = {
	FlingEnabled = false; -- THIS ISN'T GOING TO WORK WITH RECLOVR!
	ScanFling = false; -- THIS ISN'T GOING TO WORK WITH RECLOVR!
	ScanFlingRange = 5; -- THIS ISN'T GOING TO WORK WITH RECLOVR!
}
local NetOptions = {
...
local vrs=game:FindFirstChildOfClass("VRService")
...
local AccessorySettings = {
	LeftArm		= "Extra Left hand (Blocky)_white"; -- Name of hat used as this limb
	RightArm	= "Extra Right hand (Blocky)_white"; -- Name of hat used as this limb
	LeftLeg		= "MeshPartAccessory"; -- Name of hat used as this limb
	RightLeg	= "Unloaded head"; -- Name of hat used as this limb
	Torso		= ""; -- Name of hat used as this limb
	...
	LimbOffset	= CFrame.Angles(math.rad(-120), 0, 0); -- Don't touch
}
			...
			if RightHipHandle then
				...
				RightLegGrip.C0 =
					RightLegGrip.C0:Lerp(
						WeldBase.CFrame:ToObjectSpace(Positioning * angles(rad(90),rad(90),rad(0))),
						Smoothness
					)
			...
			elseif RightHandle and UserCFrame == Enum.UserCFrame.RightHand and AccessorySettings.RightArm then
				...
				if not VRReady then
					...
					if Point2 then
						...
						VirtualRig.RightUpperArm.Aim.CFrame = Camera.CFrame * angles(rad(90),rad(0),rad(0))
			...
			elseif LeftHandle and UserCFrame == Enum.UserCFrame.LeftHand and AccessorySettings.LeftArm then
				...
				if not VRReady then
					...
					if Point1 then
						...
						VirtualRig.LeftUpperArm.Aim.CFrame = Camera.CFrame * angles(rad(90),rad(0),rad(0))
